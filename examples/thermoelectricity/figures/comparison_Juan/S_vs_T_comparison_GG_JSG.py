...
######################################################
data = np.loadtxt("SLIO_x=0.206_G37-S10_Tcooldown_Seebeck_NSYM_B=0T_2026-04-14.txt", dtype="float", comments="#", delimiter=',')
...
color = "#000000"
# color = '#00EFD0'
line = axes.plot(x, y, label = r"Juan")
plt.setp(line, ls ="-", c = color, lw = 2, marker = "", mfc = color, ms = 7, mec = color, mew= 2)
######################################################

######################################################
data = np.loadtxt("../../data_analyzed/Tsweep_Seebeck_B=0T_SLIOx=0.158G37S14_2026-04-14.txt", dtype="float", comments="#", delimiter=',')
...
color = '#FF0000'
# color = '#00EFD0'
line = axes.plot(x, y, label = r"Gael")
plt.setp(line, ls ="-", c = color, lw = 2, marker = "", mfc = color, ms = 7, mec = color, mew= 2)
######################################################
...
#############################################
axes.set_xlim(0, 30)   # limit for xaxis
axes.set_ylim(-15,0) # leave the ymax auto, but fix ymin
# axes.set_xscale('log')
# axes.set_yscale('log')
axes.set_xlabel(r"$T$ ( K )", labelpad = 8)
axes.set_ylabel(r"$S_{\rm AC}$ ( $\mu$V / K$^2$ )", labelpad = 8)
