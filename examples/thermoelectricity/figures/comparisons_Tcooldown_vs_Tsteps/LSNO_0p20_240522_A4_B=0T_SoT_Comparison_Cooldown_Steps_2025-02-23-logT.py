...
fig.text(0.79,0.80, r"$B$ = 0T", ha = "right")
#############################################
...
color = '#000000'
# color = '#00EFD0'
line = axes.plot(T_0, S_AC*1e6 / T_0, label = r"Cooldown AC")
plt.setp(line, ls ="-", c = color, lw = 2, marker = "", mfc = color, ms = 7, mec = color, mew= 2)
######################################################

# Steps AC //////////////////////////////////////////////////////////////////////////////
######################################################
data = np.loadtxt("../../data_analyzed/LSNO_0p20_240522_A4_Tsteps_Seebeck_B=0T_2025-02-23.txt", dtype="float", comments="#", delimiter=',')
...
color = '#FF0000'
# color = '#00EFD0'
line = axes.plot(Tav, S_AC*1e6 / Tav, label = r"Steps AC")
...
#############################################
axes.set_xlim(10,220)   # limit for xaxis
axes.set_ylim(-0.1,0.1) # leave the ymax auto, but fix ymin
axes.set_xscale('log')
# axes.set_yscale('log')
axes.set_xlabel(r"$T$ ( K )", labelpad = 8)
axes.set_ylabel(r"$S$ / $T$ ( $\mu$V / K$^2$)", labelpad = 8)
