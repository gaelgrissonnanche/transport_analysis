...
class ResistivityTemp:
    ...
    ## Methods >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    def load_info(self, prefix="info_tsweep_"):
        """
        Load the measurement pickle for the corresponding samplelabel. It contains
        the info regarding the measurements. The keys of info are
        (Field/Temperature, Date), and return for each couple of
        (Field/Temperature, Date) a particular dictionnary for each
        ...
        """
        with open(prefix + self.samplelabel + ".pkl", "rb") as f: # 'w' means (over)write or create if doesn't exist
    ...
    def clean_up(self, clean_against="T"):
        """
        Clean the data by:
    ...
    def __add__(self, obj_n):
        ...
        sym_obj = ResistivityTemp(self.samplelabel, self.date, np.abs(self.field))
        sym_obj.L = self.L
        sym_obj.w = self.w
        sym_obj.t = self.t
        ## Load the data
...
class ResistivityField(ResistivityTemp):
    def __init__(self, samplelabel, date, temperature, measurement_type="FS_rho"):
        super().__init__(samplelabel, date, temperature, measurement_type)
...
class HallEffectTemp(ResistivityTemp):
        ...
        def __sub__(self, obj_n):
            """
            When doing objxx_p - object_n = obj_sym.
            Create a new HalleffectTemp instance with Rxy, Ixy and rhoxy symmetrized
            - obj_n is the HalleffectTemp instance for negative fields
            """
            ## Creates a new object from the sum of the positive and negative field objects
            sym_obj = HallEffectTemp(self.samplelabel, self.date, np.abs(self.field))
            sym_obj.t = self.t
            ## Load the data
            T_p     = self.data["T"]
            Rxy_p   = self.data["Rxy"]
