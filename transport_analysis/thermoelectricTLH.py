...
class SeebeckTempTLH:
    ...
    def load_file(self, delimiter=",", skiprows=0):
        ...
        self.data_raw = np.loadtxt(self.folder_raw + "/"
                                   + self.file_raw,
    ...
    def extract_raw_data(self, data_raw):
        ...
        if self.use_DC == True:
            ...
            ## DC OFF
            data["Vp_DC_OFF"] = np.ones_like(data["T0"]) * self.info["DC_OFF"]["Vp"] # DC voltage of the T+ thermocouple, HEAT OFF
            data["Vm_DC_OFF"] = np.ones_like(data["T0"]) * self.info["DC_OFF"]["Vm"] # DC voltage of the T- thermocouple, HEAT OFF
            data["Vs_DC_OFF"] = np.ones_like(data["T0"]) * self.info["DC_OFF"]["Vs"] # DC voltage of the T- thermocouple, HEAT OFF
            ## Remove background
    ...
    def _ac_temp_thermocouples(self, T0, dV_AC, T_DC):
        ...
        for i in range(len(T0)):
             T_AC[i] = brentq(lambda x: E_ther(x+T_DC[i]) - dV_AC[i] - E_ther(T_DC[i]), 0, 1000)
        return T_AC
...
class SeebeckFieldTLH(SeebeckTempTLH):
